...
import { Card, Button, Table, Modal, Badge, Spinner, apiClient } from '@medisync/ui';
import { Upload, Eye, FileDigit, Calendar, FlaskConical, CornerDownRight } from 'lucide-react';
...
export const PrescriptionsPage: React.FC = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Append newly digitized prescription to the view stack
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '28px', alignItems: 'start' }}>
        <Card style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', lineHeight: 1.4 }}>
            Upload an image (PNG/JPEG) of your doctor's handwritten prescription prescription. Our AI service uses high-precision Tesseract OCR engines to digitize the salts instantly.
          </p>
          ...
          <label style={{
            ...
            {uploading ? (
              <>
                ...
                <span style={{ fontSize: '0.6875rem', color: 'var(--text-muted)', marginTop: '4px' }}>Running Tesseract character extraction</span>
              </>
            ) : (
              <>
                ...
                <span style={{ fontSize: '0.6875rem', color: 'var(--text-muted)', marginTop: '4px' }}>Supports JPEG, PNG up to 10MB</span>
              </>
            )}
          </label>
        </Card>
