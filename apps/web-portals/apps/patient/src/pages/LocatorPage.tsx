...
import { Card, Input, Button, Badge, Spinner, EmptyState, apiClient } from '@medisync/ui';
...
export const LocatorPage: React.FC = () => {
  ...
  const fetchLocations = async () => {
    ...
    try {
      ...
      // Seed fallback rating for score display if backend didn't supply one
      const rawData = res.data.data || [];
      const calculated = rawData.map((item: PharmacyStock) => {
        const trust = parseFloat(item.trustRating || '4.5');
        // Let's emulate our SRS pricing scoring if score isn't already set
        const pScore = 80.0; // price ratio mock
        const qScore = trust * 20; // 0-100 rating conversion
      ...
      // Sort by score descending (highest savings ranking first)
