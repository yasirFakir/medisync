...
export const ProfilePage: React.FC = () => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to update admin profile parameters.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '28px', alignItems: 'start' }}>
      ...
      <Card style={{ padding: '32px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '20px' }}>
        ...
        <div>
          ...
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>System Administrator Profile</p>
        </div>
      ...
      <Card style={{ padding: '32px' }}>
        <h3 style={{ fontSize: '1.125rem', fontWeight: 700, marginBottom: '24px' }}>Edit Admin coordinates</h3>
        ...
        {success && (
          <div style={{
            ...
            <CheckCircle size={16} />
            Admin coordinates updated successfully
          </div>
        )}
        ...
        <form onSubmit={handleUpdate} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <Input
            label="Registered Phone Coordinate"
