...
export const ProfilePage: React.FC = () => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to update pharmacy store parameters.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '28px', alignItems: 'start' }}>
      ...
      <Card style={{ padding: '32px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '20px' }}>
        <Avatar name={user?.fullName || 'Pharmacy'} size={80} />
        <div>
          <h3 style={{ fontWeight: 800, fontSize: '1.25rem', marginBottom: '4px' }}>{user?.fullName}</h3>
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>Partner Pharmacy Node</p>
        </div>
      ...
      <Card style={{ padding: '32px' }}>
        <h3 style={{ fontSize: '1.125rem', fontWeight: 700, marginBottom: '24px' }}>Edit Store coordinate parameters</h3>
        ...
        {success && (
          <div style={{
            ...
            <CheckCircle size={16} />
            Pharmacy parameters updated successfully
          </div>
        )}
        ...
        <form onSubmit={handleUpdate} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <Input
            label="Registered Store Phone Parameter"
